...
class MyRobot1(RCJSoccerRobot):
    BallPositionController = PID(1, 10, 0.0005, 10, -10)
    BallAngleCotrnoller = PID(20, 10, 0.00075, 10, -10)
    GoalPositionCotrnoller = PID(20, 0, 0.002, 0, 0)
    GoalAngleCotrnoller = PID(2, 30, 0.000045, 20, -20)
        ...
        while self.robot.step(TIME_STEP) != -1:
            if self.is_new_data():
                ...
                e_ball_theta = math.atan2(
                    ball_data["direction"][0], ball_data["direction"][1] - 0.2) - (math.pi / 2)
                ...
                if abs(e_ball_position) > 0.01 and heading_stage == 0:
                    w = v_ball_theta
                ...
                self_goal_distance = math.sqrt((robot_pos[1] - 0.55)**2 + (robot_pos[0])**2)
                if self_goal_distance < 0.3 and ball_data['strength'] > 10: 
                    vr = 0
                    vl = 0
                ...
                if heading_stage != 0:
                    ...
                    if heading_stage_counter % 40 == 0 and heading_stage == 2:
                        heading_stage = 0
                    if heading_stage_counter % 80 == 0 and heading_stage == 2:
                        heading_stage = 0
